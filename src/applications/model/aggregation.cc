...
AggregationApplication::AggregationApplication() : alive(false)
{
	...
	this->m_videostreams = VideoStreamDash::getInstance();
}
...
void AggregationApplication::ConnectionAccepted (Ptr<Socket> socket, const Address& address)
{
	...
	cout << "AggregationServer(" << socket << ") " << Simulator::Now () 
			  << " Successful socket creation Connection Accepted From " << address << endl;
...
void AggregationApplication::HandleIncomingData(Ptr<Socket> socket)
{
	...
	cout << "================================================================================\n";
    fprintf(stderr, "Aggregation : HandleIncomingData - connection with Aggregation reseceived %s\n", s.c_str());
    
    HandleReadyToTransmit(socket, s);
}

void AggregationApplication::HandleReadyToTransmit(Ptr<Socket> socket, std::string &video)
{
	std::cout << "AggregationServer " << Simulator::Now () << " Socket HandleReadyToTransmit (AggregationApplication Class) From " << socket << std::endl;
		
//	this->m_videostreams->printLst();
	
	std::string requestString = this->m_videostreams->getIps(video);
