...
DASHFakeServerApplication::StartApplication (void)
{
    ...
    GetNode()->GetObject<Ipv4>()->GetAddress(1,0).GetLocal().Print(oss);
    ipv4str = oss.str();
...
//        std::cout << "----------------------------------------------------------------------------" << std::endl;
//        std::cout << mpdData << std::endl;
//        std::cout << "----------------------------------------------------------------------------" << std::endl;
//        getchar();

        // compress
        std::string compressedMpdData = zlib_compress_string(mpdData);
        fprintf(stderr, "DashServer(%d,%s,%f): Size of compressed = %ld, uncompressed = %ld\n", GetNode()->GetId(), ipv4str.c_str(), Simulator::Now().GetSeconds(), compressedMpdData.length(), mpdData.length());
        ...
        fprintf(stderr, "DashServer(%d,%s,%f): Adding '%s' to m_fileSizes with size %ld\n", GetNode()->GetId(), ipv4str.c_str(), Simulator::Now().GetSeconds(), SSMpdFilename.str().c_str(), compressedMpdData.size());
...
{
    NS_LOG_FUNCTION (this << socket << address);
    fprintf(stderr, "DASH Fake Server(%d, %s): Connection Accepted!\n", GetNode()->GetId(), ipv4str.c_str());
