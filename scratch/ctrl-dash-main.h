...
void DashController::onAddContainer(unsigned from, unsigned to, int content, int userId)
{
	...
	// cout << "CONEXAO " << m_group_i->getId() << " (" << actualNode << "," << nextNode << ") NAO ALLOCADA "
	// 			<< network->CapacityLink(actualNode, nextNode) << endl;
	// for (size_t i = 0; i < groups_i.size(); i++)
	// 	cout << "groups_i -> " << groups_i[i] << endl;
	// for (size_t i = 0; i < free_groups_index.size(); i++)
	// 	cout << "free_groups_index " << i << " -> " << free_groups_index[i] << endl;
	// getchar();
